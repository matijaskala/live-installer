...
class InstallerWindow:
    ...
    def __init__(self, fullscreen=False):
        ...
        self.wizard_pages[self.PAGE_INSTALL] = WizardPage(_("Installing Gentoo..."), "install.png")
        ...
        self.wTree.get_widget("button_add_partition").connect("clicked", partitioning.add_partition_dialog, None, None)
        self.wTree.get_widget("button_edit_partition").connect("clicked", partitioning.edit_partition_dialog, None, None)
        self.wTree.get_widget("button_remove_partition").connect("clicked", partitioning.remove_partition_dialog, None, None)
        ...
        if(not goback):
            if(sel == self.PAGE_LANGUAGE):
                ...
                if not found_root_partition:
                    ErrorDialog(_("Installation Tool"), "<b>%s</b>" % _("Please select a root (/) partition."), _("A root partition is needed to install Gentoo on.\n\n - Mount point: /\n - Recommended size: 30GB\n - Recommended filesystem format: ext4\n "))
                    return
